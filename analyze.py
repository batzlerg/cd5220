...
def bookmarks():
    ...
    display_cols = [c for c in display_cols if c in bookmarked.columns]
    
    result = bookmarked[display_cols].sort_values('datetime', ascending=False)
    
    # Add summary statistics
    print(f"Total bookmarks: {len(bookmarked)}")
    if 'unique_chars_count' in bookmarked.columns:
        avg_variety = bookmarked['unique_chars_count'].mean()
        print(f"Avg character variety: {avg_variety:.1f}")
    if 'spatial_patterns' in bookmarked.columns:
        # Count pattern occurrences
        pattern_counts = {}
        for patterns in bookmarked['spatial_patterns']:
            if isinstance(patterns, list):
                for p in patterns:
